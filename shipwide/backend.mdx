...
deployment serves one product. The code is the source of truth.

## Module architecture

Three layers per feature — keep all complete:
...
<Note>
  Every exported service function, handler, schema and type is a **ready-to-use building
  block**. An exported handler that no default route mounts is **not** dead code — wire it up
  when you need it. Remove something only if it's broken, a true duplicate, or an architecture
  violation.
</Note>
...
<Warning>
  Don't reintroduce a password flow — passwordless is a deliberate product decision.
</Warning>
